...
TEST_P(TransientLocalPermutationTest, PublishWithNoSubs)
{
    ...
    auto executor =
    std::make_unique<rclcpp::experimental::executors::EventsExecutor>(std::move(events_queue),
    false, rclcpp::ExecutorOptions());
    ...
    const std::string & prefix)
    {
      for (size_t i = 0; i < subs_use_ipc.size(); i++) {
        ...
        sub_options.use_intra_process_comm =
          use_ipc ? rclcpp::IntraProcessSetting::Enable : rclcpp::IntraProcessSetting::Disable;
